...
<body>
    <div class="container mt-2">
        ...
        <form action="{{ route('mutasibarang.update',$mutasibarang->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-5">
                            <input type="text" readonly name="noBukti" class="form-control" placeholder="No. Bukti" value="{{ old('noBukti', $mutasibarang->noBukti) }}">
                        </div>
                        @error('noBukti')
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-5">
                            <input type="date" name="tanggal" class="form-control" placeholder="Tanggal Mutasi"  value="{{ old('tanggal', $mutasibarang->tanggal) }}">
                        </div>
                        @error('tanggal')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="col-sm-5">
                            <select class="form-control" name="isMasuk">
                                @foreach ($statusBarang as $key => $status)
                                    <option value="{{ $key }}" {{ $mutasibarang->isMasuk == $key ? "selected" : "" }}>{{ $status }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('isMasuk')
            ...
            <div id="formMutasiBarang" class="row mt-5">
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="input-group">
                            ...
                            <button class="btn btn-outline-primary" id="tambahbarang"> tambah barang</button>
            ...
            <div class="row mt-4">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <table class="table table-bordered">
                        ...
                        <tbody id="listBarang">
                            @if (old('idBarang',$detailMutasi))
                                @foreach (old('idBarang',$detailMutasi) as $key => $mutasi)
                                    <tr class="barangTermutasi form-group-{{ old('idBarang.'.$key) ? old('idBarang.'.$key) : $mutasi->barang->id }}">
                                        <td>{{ $key+1 }} <input type="hidden" name="idBarang[]" value="{{ old('idBarang.'.$key) ? old('idBarang.'.$key) : $mutasi->barang->id }}" /> </td>
                                        <td>{{old('namaBarang.'.$key) ? old('namaBarang.'.$key) : $mutasi->barang->kodeBarang."-".$mutasi->barang->namaBarang}} <input type="hidden" name="namaBarang[]" value="{{ old('namaBarang.'.$key) ? old('namaBarang.'.$key) :$mutasi->barang->namaBarang }}"/></td>
                                        <td><input type="text" name="quantity[]" value="{{ old('quantity.'.$key) ? old('quantity.'.$key)  : $mutasi->quantity }}"/></td>
                                        <td><button class="btn btn-danger" onclick="hapusBaris(event,'<?php echo old('idBarang.'.$key) ? old('idBarang.'.$key) : $mutasi->barang->id  ?>')">-</button></td>
                                    </tr>
                                @endforeach
                            @else
                                <tr id="empty-row">
            ...
            <button type="submit" class="btn btn-primary ml-3">Update Mutasi</button>
        </form>
    </div>
</body>
